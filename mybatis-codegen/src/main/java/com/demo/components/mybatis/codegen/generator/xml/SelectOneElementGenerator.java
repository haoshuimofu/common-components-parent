...
public class SelectOneElementGenerator extends AbstractXmlElementGenerator {
    ...
    public void addElements(XmlElement parentElement) {
        // 如果表主键只有一列，查询条件不包含主键，此时查询用selectById就可以了，与此同时如果表仅仅只有一列主键，那就不用生成sql了
        ...
        selectClause.append("SELECT ")
                .append(CodegenConstants.XML_ELEMENT_BASE_COLUMN_LIST)
                .append(" FROM ")
        ...
        List<IntrospectedColumn> columns = introspectedTable.getPrimaryKeyColumns().size() == 1 ?
                introspectedTable.getNonPrimaryKeyColumns() : introspectedTable.getAllColumns();
