...
public class SelectByIdElementGenerator extends AbstractXmlElementGenerator {
    ...
    public void addElements(XmlElement parentElement) {
        ...
        // parameterType设置看主键类型
        if (primaryKeyColumns.size() == 1) {
            answer.addAttribute(new Attribute("parameterType",
                    primaryKeyColumns.get(0).getFullyQualifiedJavaType().getFullyQualifiedName()));
        } else {
        ...
        for (int i = 0; i < primaryKeyColumns.size(); i++) {
            ...
            selectCaluse.append(MyBatis3FormattingUtilities.getEscapedColumnName(column));
            selectCaluse.append(" = ");
