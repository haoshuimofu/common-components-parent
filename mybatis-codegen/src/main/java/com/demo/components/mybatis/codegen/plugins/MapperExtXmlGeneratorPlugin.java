package com.demo.components.mybatis.codegen.plugins;
 ...
 */
public class MapperExtXmlGeneratorPlugin extends PluginAdapter {

    private final ShellCallback shellCallback;
    private Logger logger = LoggerFactory.getLogger(MapperExtXmlGeneratorPlugin.class);

    public MapperExtXmlGeneratorPlugin() {
    ...
    public List<GeneratedXmlFile> contextGenerateAdditionalXmlFiles(IntrospectedTable introspectedTable) {
        ...
        File mapperExtFile = new File(targetProject + "/"
                + targetPackage.replaceAll("\\.", "/")
                + "/" + mapperExtFileName);
        // 如果ModelMapperExt.xml文件已存在, 直接跳过
        if (mapperExtFile.exists()) {
            logger.info(">>> [{}]文件已存在!!!", mapperExtFile.getPath());
        ...
        GeneratedXmlFile generatedExtXmlFile = new GeneratedXmlFile(document, mapperExtFileName,
                targetPackage, targetProject, true, context.getXmlFormatter());
        return Collections.singletonList(generatedExtXmlFile);
    }
